...
frontend:
  ...
  - task: "Worker Profile — Availability rules + single-select Job title + A-Z sort"
    implemented: true
    working: "NA"
...
agent_communication:
    - agent: "main"
      message: "Implemented In-app Notifications Inbox. Backend: new /api/notifications GET (list, ?limit,?unread_only), GET /api/notifications/unread-count, POST /api/notifications/mark-read {ids?, all?}, DELETE /api/notifications/{id}, DELETE /api/notifications (clear all). Helper create_notification() + notify_admins() inserted at trigger points: application submitted → notify job poster; application accepted/rejected → notify worker; attendance check_in/check_out → notify job poster; complaint filed → notify all admins; complaint resolved/rejected → notify complainant; escrow release → notify worker (wallet credit); leave request → notify approver; leave decision → notify worker; admin verify/suspend/unsuspend → notify user. New Mongo indexes on notifications collection. Frontend: new /app/notifications.tsx inbox screen with unread badge, mark-all-read, clear-all, long-press to delete, tap-to-deep-link. New NotificationBell component with unread badge and 45s polling. Bell added to Worker/Client/Contractor home header AND Admin dashboard hero. Please test: (1) unread-count endpoint returns {count:N}. (2) File a complaint as worker → admin gets notification (type=complaint_new). (3) Admin resolves → worker gets notification (complaint_resolved). (4) Worker applies to a job → poster gets application_new. (5) Poster accepts application → worker gets application_accepted + wage flow. (6) Mark all read decrements counter to 0. (7) Delete /api/notifications/{id} for other user's notification returns 404. (8) DELETE /api/notifications clears all for current user. Frontend UI: bell shows red badge with unread count; tapping opens inbox; unread items have orange left-border; long-press deletes with confirm dialog."
    - agent: "main"
